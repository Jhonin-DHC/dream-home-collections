...
export function PostCard({ post }: { post: PublicPost }) {
  return (
    <Link href={`/${post.slug}`} className="group block overflow-hidden border border-[var(--stone)] bg-white">
      {post.featuredImage ? (
        <div className="relative h-52 bg-[var(--stone)]">
          <RemoteImage src={post.featuredImage} alt={post.title} className="object-cover transition duration-500 group-hover:scale-105" sizes="(max-width: 768px) 100vw, 50vw" />
        </div>
      ) : null}
      <div className="p-5">
