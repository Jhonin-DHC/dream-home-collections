...
export const PRIMARY_SERVICE_COUNTIES = ["Collin", "Denton", "Dallas", "Tarrant"] as const;
...
export const EXTENDED_SERVICE_COUNTIES = [...EXTENDED_SERVICE_COUNTIES_RAW].sort((a, b) => a.localeCompare(b));

export const ALL_SERVICE_COUNTIES = [...PRIMARY_SERVICE_COUNTIES, ...EXTENDED_SERVICE_COUNTIES] as const;
...
export function buildTexasCountyMap(args: {
  ...
  const countiesCollection = feature(topology as never, topology.objects.counties as never) as unknown as {
    features: Array<{
      id?: string | number;
  ...
  const statesCollection = feature(topology as never, topology.objects.states as never) as unknown as {
    features: Array<{
      id?: string | number;
      geometry: unknown;
      type: string;
  ...
  const texasCountyFeatures = countiesCollection.features.filter((item) => String(item.id ?? "").startsWith(TEXAS_STATE_FIPS));
  const texasStateFeature = statesCollection.features.find((item) => String(item.id ?? "") === TEXAS_STATE_FIPS) ?? null;
